...
### 편미분
인수들의 제곱 합을 계산하는 아래와 같은 함수가 있다고 가정해보자.
$$
f(x_0,x_1) = x{_0}{^2} + x{_1}{^2}
$$

이 식은 python으로 아래와 같이 구현될 것이다.
인수 x는 넘파이 배열이다.
```python
def func(x):
    return x[0]** 2 + x[1]**2
...
이 함수를 그래프로 그리면 3차원 그래프가 그려진다.
<figure>
    <img src="/images/3d_1.png" alt="3차원 그래프 모습" />
    <figcaption>3차원 그래프의 모습</figcaption>
</figure>

이 식을 미분하면, '어느 변수에 대한 미분인지'에 따라 결과가 달라질 수 있다.  
즉 $x_0$와 $x_1$ 중 어느 변수에 대한 미분이냐를 구분해야 한다. 덧붙여 이와 같이 변수가 여러 개인 함수에 대한 미분을
<b>편미분</b>이라고 한다.

편미분은 수식으로 $\frac{\partial f}{\partial x_0}$이나 $\frac{\partial f}{\partial x_1}$처럼 작성한다.

편미분은 미분하려는 변수를 제외하고선 나머지를 전부 <b>상수</b> 취급한다. 상수를 미분하면 0이 된다.  

편미분을 python으로 구현하면 어떻게 될까? $x_0 = 3, x_1 = 4$일 때 편미분 $\frac{\partial f}{\partial x_0}$을 구하는 python은 아래와 같이 작성할 수 있다.

```python
def function_tmp(x1):
    return 3.0 * 2.0 + x1*x1

diff(function_tmp, 4.0)
# 결과는 약 7.9999...19
```
이 문제는 변수가 하나인 함수를 정의하고 그 함수를 미분하는 형태로 풀면 된다.

## 기울기
앞에서 $x_$와 $x_1$의 편미불을 변수별로 따로 계산했다. 만약 $x_0$와 $x_1$의 편미분을 동시에 계산하고 싶다면 어떻게 해야 할까?
가령 $x_0 = 3, x_1 = 4$일 때 $(x_0, x_1)$ 양쪽의 편미분을 묶어서 $(\frac{\partial f}{\partial x_0},\frac{\partial f}{\partial x_1})$처럼 모든 변수의 편미분을 벡터로 정리한 걸 <b>기울기</b>라고 한다.

기울기를 구하는 python 코드는 아래와 같이 구현할 수 있다.
...
내용이 좀 복잡해 보이지만, 동작 방식은 변수가 하나일 때의 수치 미분과 거의 동일하다. `np.zeros_like(x)`는 $x$와 형상이 같고 그 원소가 모두 0인 배열을 만든다.

`diff(f,x)`의 인수인 함수 `f`는 넘파이 배열 `x`의 모든 원소에 대해 수치 미분을 진행한다. 이 함수를 사용해 실제 기울기를 계산해보자.  
$(3,4)$에서의 기울기를 예시로 보자.

```python
diff(func, np.array([3.9, 4.0]))
# array([6., 8.])
```
점 $(3,4)$의 기울기는 $(6,8)$이라는 결과가 나왔다. 여기서 이 기울기는 무얼 의미할까? 기울기 결과에 마이너스를 붙인 벡터를 그려보자.

<figure>
    <img src="/images/grad_1.png" alt="기울기 그래프 모습" />
    <figcaption>기울기 그래프의 모습</figcaption>
</figure>

기울기 그림은 위처럼 방향을 가진 벡터로 그려진다. 이 그림을 보면 기울기는 함수의 '가장 낮은 곳(최솟값)'을 가리키는 것처럼 보인다.
또한 '가장 낮은 곳'에서 멀어질수록 화살표의 크기가 커지는 것도 볼 수 있다.

그래프 상으로 기울기는 가장 낮은 장소를 가리키나, 실제로 반드시 그렇다곤 할 순 없다. 사실 <b>기울기는 각 지점에서 낮아지는 방향</b>을 가리킨다.

좀 더 정확히 말하자면 <b>기울기가 가리키는 쪽은 함수의 출력 값을 가장 크게 줄이는 방향</b>이라고 할 수 있다.

### 경사법(경사 하강법)
머신러닝 문제 대부분은 학습 단계에서 최적의 매개변수를 찾아낸다. 신경망 역시 최적의 매개변수(가중치와 편향)를 학습 시에 찾는다.

여기서 최적이란 손실 함수가 최솟값이 될 때의 매개변수이다. 그러나 일반적인 문제의 손실 함수는 매우 복잡하다. 매개변수 공간이 너무 넓어서 어디가
최솟값이 되는 곳인지를 짐작할 수 없다. 이러한 상황에선 기울기를 잘 활용해 최솟값(또는 가능한 한 작은 값)을 찾으려는 기법이 경사법이다.

여기서 중요한 건 각 지점에서 함수의 값을 낮추는 방안을 제시하는 게 기울기(음의 기울기)라는 것이다. 그러나 기울기가 가리키는 곳에 정말 함수의 최솟값이
있는지, 즉 그족이 정말로 나아갈 방향인지는 보장할 수 없다. 실제로 복잡한 함수에선는 기울기가 가리키는 방향에 최솟값이 없는 경우가 대부분이다.

함수가 극솟값, 최솟값 또 안장점이 되는 장소에서는 기울기가 0이다. 극솟값은 국소적인 최솟값, 즉 한정된 범위에서의 최솟값인 점이다. 안정저믄 어느 방향에서 보면 극댓값이고 다른 방향에서 보면 극솟값이 되는 점이다. 경사법은 기울기가 0인 장소를 찾으나, 그곳이 반드시 최솟값이라고는 할 수 없다. 또 복잡하고 찌그러진 모양의 함수라면 대부분 평평한 곳으로 파고들면서 플래토라는 학습이 진행되지 않는 정체기에 빠질 수 있다.

기울어진 방향이 꼭 최솟값을 가리키는 건 아니나, 그 방향으로 가야 함수의 값을 줄일 수 있다. 따라서 최솟값이 되는 장소를 찾는 문제(혹은 가능한 작은 값이 되는 장소를 찾는 문제)에서는 기울기 정보를 단서로 나아갈 방향을 정해야 한다.

경사법은 현 위치에서 기울어진 방향으로 일정 거리만큼 이동한다. 그런 다음 이동한 곳에서도 마찬가지로 기울기를 구하고, 또 그 기울어진 방향으로 나아가는 걸 반복한다.

이런 식으로 함수의 값을 점차 줄이는 기법을 경사법(gradient method)라고 부른다. 경사법은 머신러닝 모델을 최적화하는 데에 사용하는 기법이다. 특히 신경망 학습에서 경사법이 자주 사용된다.
...
$\eta$(에타) 기호는 갱신하는 양을 나타낸다. 이를 신경망 학습에서는 <b>학습률</b>이라고 한다. 한 번의 학습으로 얼마만큼 학습해야 할지, 즉 매개변수 값을 얼마나 갱신하느냐를 정하는 게 학습률이다.

위의 식은 1회에 해당하는 갱신이고 이 단계를 반복한다. 변수의 값을 갱신하는 단계를 반복해 서서히 함수 값을 줄이는 것이다. 변수의 개수가 늘어도 각 변수의 편미분 값으로 갱신하게 된다.

이러한 경사법을 사용하려면 $\eta$ 값을 0.01이나 0.0001 등으로 미리 정해두어야 한다. 일반적으로 이 값이 너무 크거나 작으면 '좋은 장소'를 찾아갈 수 없다.  
신경망 학습에서는 일반적으로 이 학습률 값을 변경하면서 올바르게 학습하고 있는지 확인하면서 진행한다.

경사 하강법은 아래와 같이 쉽게 구현할 수 있다.

```python
def grad_des(f, init_x, lr=0.01, step=100):
    x = init_x
...
인수 `f`는 최적화하려는 함수, `init_x`는 초깃값, `lr`은 학습ㄹㄹ, `step`은 경사법에 따른 반복 횟수를 의미한다. 함수 기울기는 앞서 정의했던 기울기 함수인 `diff`로 구하고 그 기울기에 학습률을 곱한 값으로 x에서 빼 함수에 대한 극소, 최솟값을 구한다.

경사법으로 함수 $f(x_1,x_2) = x{_1}{^2} + x{_2}{^2}$의 최솟값을 구해보자.
```python
def func(x):
    return x[0]**2 + x[1]**2

init_x = np.array([(3.0,4.0)])
grad_des(func, init_x=init_x, lr=0.1, step=100)
# array([-6.110793e-10, 8.14814391e-10])
```

여기에서 초깃값을 (-3.0, 4.0)으로 설정한 뒤 경사법을 사용해 최솟값 탐색을 시작한다.  
최종 결과는 (-6.1e-10, 8.1e-10)으로 수학적 최솟값인 (0,0)과 거의 일치하는 걸 볼 수 있다.

경사법을 이용한 이 갱신 과정을 그래프로 그리면 아래와 같다. 값이 가장 낮은 장소인 '원점'에 점차 가까워져 가는 걸 볼 수 있다.

<figure>
    <img src="/images/grad_2.png" alt="기울기 그래프 모습" />
    <figcaption>기울기 그래프의 모습</figcaption>
</figure>

학습률인 `lr`이 너무 크거나 작으면 좋은 결과를 얻을 수 없다는 설명을 했었다.  
너무 큰 예(`lr=10`)와 작은 예(`lr=1e-10`)를 결과는 각각 너무 큰 값으로 발산(-25898e+13, -1.2952ㄷ+12)하고 거의 갱신되지 않는다(-2.99999, 3.99999).

학습률 같은 매개변수를 <b>하이퍼파라미터(hyper paramter)</b>라고 한다. 이는 가중치와 편향 같은 신경망의 매개변수와는 조금 다르다.
신경망의 가중치 매개변수는 훈련 데이터와 학습 알고리즘에 의해서 <b>자동으로 획득</b>되는 변수인 반면, 하이퍼파라미터는 <b>사람이 직접 설정</b>하는 매개변수이다. 일반적으로 이 하이퍼파라미터들은 여러 후보 값 중에서 시험을 통해 가장 잘 학습하는 과정을 거쳐야 한다.

### 신경망에서의 기울기
신경망 학습에서도 기울기를 구해야 한다. 여기서 말하는 기울기는 <b>가충치 매개변수에 대한 손실 함수의 기울기</b>다. 예를 들어 형상이 $2*3$, 가중치가 $W$, 손실함수가 $L$인 신경망을 생각해보자. 이 경우 경사는 $\frac{\partial L}{\partial W}$로 나타낼 수 있다.
