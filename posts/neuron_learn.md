...
## 기울기
앞에서 $x_0$와 $x_1$의 편미불을 변수별로 따로 계산했다. 만약 $x_0$와 $x_1$의 편미분을 동시에 계산하고 싶다면 어떻게 해야 할까?
...
인수 `f`는 최적화하려는 함수, `init_x`는 초깃값, `lr`은 학습률, `step`은 경사법에 따른 반복 횟수를 의미한다. 함수 기울기는 앞서 정의했던 기울기 함수인 `diff`로 구하고 그 기울기에 학습률을 곱한 값으로 x에서 빼 함수에 대한 극소, 최솟값을 구한다.
