...
200원은 $×1.1$ 노드를 거쳐 220원이 되고 이게 최종 정답이다.

위 차트에선 $×2$와 $×1.1$을 각각 하나의 연산으로 취급했지만, 곱셉인 $×$만을 연산으로 생각할 수도 있다.
...
예를 들어 복잡한 계산 결과와 호두 2개인 200원을 거기에 더하는 그래프를 상상해보자. 복잡한 계산의 결과가 얼마가 됐던 '덧셈 노드'는 그냥 <b>두 숫자(a + 200)</b>를 더할 뿐이다.
...
#### 왜 계산 그래프를 쓰는가
지금까지 살펴본 계산 그래프 예시로 이점을 알 수 있다. 

하나는 '국소적 계산'이다. 전체 연산이 아무리 복잡해도 각 노드 입장에선 국소적인 연산만을 수행해 문제를 단순화시킨다.

또 다른 이점은 <b>계산 그래프는 중간 계산 결과를 모두 보관</b>할 수 있다는 점이다. 앞서 보여준 그림처럼 <b>중간 계산값</b>($200, 650, 715$)을 전부 알 수 있었다.
...
이 미분값은 호두 값이 '<b>정말 미세하게</b>' 상승했을 때의 최종 지불 금액 증가량을 나타낸다.
...
현재까지 계산 그래프의 순전파는 계산 결과를 왼쪽에서 오른쪽으로 전달했다. 이 순서는 일반적인 식의 전개와 동일하다.

하지만 역전파는 '국소적인 미분'을 순전파와 반대인 오른쪽에서 왼쪽으로 전달한다. 또한, 이 '국소적 미분'을 전달하는 원리는 <b>연쇄법칙</b>을 따른다.

계산 그래프를 사용한 역전파의 예시를 하나 살펴보자. $y=f(x)$라는 계산의 역전파는 아래와 같다.

```mermaid
flowchart LR
    %% 노드 정의 (양 끝의 투명한 노드로 입력/출력 지점 표현)
    Input[ ]
    Output[ ]
    NodeF((f))

    %% 스타일 정의
    style Input fill:none,stroke:none,width:0px
    style Output fill:none,stroke:none,width:0px
    style NodeF fill:#fff,stroke:#333,stroke-width:1px,font-size:20px

    %% 1. 순방향 전파 (Forward) - 검은색 얇은 선
    Input -- "x" --> NodeF
    NodeF -- "y" --> Output

    %% 2. 역방향 전파 (Backward) - 빨간색 굵은 선
    Output -- "E" --> NodeF
    NodeF -- "E ∂y/∂x" --> Input

    %% 링크 스타일링 (순서: 0,1은 순방향 / 2,3은 역방향)
    %% 역방향 링크를 빨간색, 굵게 설정
    linkStyle 2,3 stroke:#b30000,stroke-width:1px,color:#b30000,font-size:16px;
    linkStyle 0,1 stroke:#333,stroke-width:1px,font-size:16px;
```

신호 $E$에 노드의 국소적 미분(\$frac{\partial y}{\partial x}$)을 곱한 후 다음 노드로 전달해 역전파를 계산한다.

여기서 말하는 국소적 미분은 순전파 때의 $y=f(x)$ 계산의 미분을 구한다는 의미이며, 이는 $x$에 대한 $y$의 미분을 구한다는 말이다.

가령 $y=f(x)=x^2$라면 $\frac{\partial y}{\partial x} = 2x$가 된다. 그리고 이 국소적 미분을 상류에서 전달된 값(여기선 $E$)에 곱해
앞쪽 노드로 전달하는 것이다.

이것이 역전파 계산 순서인데, 이러한 방식을 따르면 목표로 하는 미분 값을 효율적으로 구할 수 있다는 점이 핵심이다.

어떻게 이러한 일이 가능한가는 연쇄법칙의 원리로 설명할 수 있다. 연쇄법칙을 설명하기 위해선 합성 함수를 이야기해야 한다.

<b>합성함수</b>란 여러 함수로 구성된 함수이다. 예를 들어 $z = (x+y)^2$라는 식은 아래의 두 식을 구성된다.
...
연쇄법칙은 <b>함성 함수의 미분에 대한 성질</b>이며, 아래와 같이 정의된다.

> 합성 함수의 미분은 합성 함수를 구성하는 각 함수의 미분 곱으로 나타낼 수 있다.
...
결론적으로 덧셈 노드의 역전파에선 <b>입력 신호(상류의 미분값)를 그냥 그대로 다음 노드로 전달</b>하면 된다.
...
곱셈 노드의 역전파는 합성 함수의 미분에 따라서 $x$는 $x$에 대한 편미분인 $y × 1$을 $y$는 $y$애 대한 편미분인 $x × 1$을 상류에서 전달받은 미분값에 곱해서 전달시킨다. 즉, <b>서로 입력 신호를 바꿔서 하류로 전달</b>한다.
...
### 활성화 함수 계층 구현하기
계산 그래프를 신경망에 적용할 때가 왔다. 여기선 신경망을 구성하는 층(계층)을 각각 하나의 클래스로 구현한다. 

먼저 활성화 함수인 ReLU와 Sigmoid 계층을 구현해보자.
...
위 식에서도 볼 수 있듯이 입력인 $x$가 0보다 클 경우 역전파는 <b>상류의 값을 그대로 하류로 흘린다</b>. 반면, 순전파 때 $x$가 0 이하면 역전파 때는 하류로
신호를 보내지 않는다. (0 보냄)
...
flowchart LR
    ...
    %% 함수 노드 (원형)
    classDef funcNode fill:#fff,stroke:#333,stroke-width:1px,min-width:50px,min-height:50px,font-size:18px;
    ...
    %% --- 링크 스타일링 ---
    %% 순방향 링크 (0,1, 4,5번): 얇은 회색
    linkStyle 0,1,4,5 stroke:#999,stroke-width:1px,color:black,font-size:16px;

    %% 역방향 링크 (2,3, 6,7번): 굵은 빨간색
    %% Case1의 역전파
    linkStyle 2,3 stroke:#b30000,stroke-width:4px,color:#b30000,font-size:16px;
    %% Case2의 역전파
    linkStyle 6,7 stroke:#b30000,stroke-width:4px,color:#b30000,font-size:18px;
```

python 코드로 구현하면 아래와 같다. `forward()`와 `backward()`는 넘파이 배열을 인수로 받는다.
...
class Relu
    ...
    def backward(self, dout):
        ...
        return dx
```
Relu 클래스는 mask라는 인스턴스 변수를 갖는다. mask는 True/False로 구성된 넘파이 배열로, 순전파의 입력인 x의 원소 값이 0 이하인 인덱스는 True, 그 외(0 초과 원소)는 False로 유지한다.

앞서 역전파 그림에서 봤듯이 순전파 때의 입력값이 0 이하일 경우 역전파 때의 값은 0이 되어야 한다. 그래서 역전파 때는 순전파 때 만들어둔 mask를 써서 mask의 원소가 True인 곳에는 상류에서 전달된
dout을 0으로 설정한다.

#### Sigmoid 게층
시그모이드 함수는 아래의 식을 의미하는 함수이다.
