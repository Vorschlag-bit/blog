...
### 1-3. 전송 데이터 눈으로 확인해보기 (RSC Payload)
앞서 '서버가 HTML'을 구워준다는 표현을 했지만 엄밀히 말하면 서버가 제공하는 건 순수 HTML은 아니다. Next.js App router로
구동되는 페이지를 켜두고 개발자 도구의 네트워크 탭을 통해 <b>RSC Payload</b>라는 직렬화된 데이터를 볼 수 있다.

<figure>
  <img src="/images/rsc_p.png" alt="rsc payload 직렬화된 내용 캡쳐">
  <img src="/images/rsc_p2.png" alt="rsc payload 직렬화 다른 부분 캡쳐">
  <figcaption>매우 복잡해보이는 직렬화된 데이터를 볼 수 있다.</figcaption>
</figure>

형태가 복잡하긴 하나 명확한 규칙이 존재한다.  
1. <b>서버 컴포넌트의 결과물</b>: `<Link>`, `<div>` 태그와 그 안의 텍스트들은 전부 다 계산된 데이터(JSON)으로 들어가 있다.  
2. <b>클라이언트 컴포넌트의 결과물</b>: `1:I[...]` 부분은 이 자리에 클라이언트 컴포넌트가 들어가야 한다고 브라우저에게 알려주는 <b>참조(PlaceHolder)</b>이다.

왜 그냥 HTML을 안 주고 이런 복잡한 직렬화된 데이터를 줄까?  
만약 서버가 쌩 HTML을 준다면, 페이지가 이동할 때마다 브라우저는 화면 전체를 새로고침해야 한다. 하지만 이런 데이터 형식으로 주면, React는 현재 화면의 상태를 유지하면서 바뀐 부분만 데이터를 바꿔 끼울 수 있다.

즉, <b>RSC Payload는 React가 DOM 트리를 재구성하기 위해 보내지는 설계도</b>인 셈이다.  
이 덕분에 Next.js는 SSR이지만 앱처럼 부드럽게 페이지가 넘어가진다. (HTML 전체 교체가 아니라 리액트가 Payload를 해석해서 병합)
