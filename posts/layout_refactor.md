...
카테고리를 수정하고 나서 전체적으로 손봐야 할 것들을 좀 정리해봤다.

수정해야 할 화면은 크게 5가지로 다음과 같았다.  
...
4. `/app/layout.js`의 header와 footer: 엄청 이상한 건 아니지만 전체적인 크기와 패딩,마진에 대한 미디어 쿼리가 필요하고 `md` 이상부터만
`SearchModal.js`를 보여주도록 수정.
...
<figure>
    <img src="/images/tab1.png" alt="수정된 메인 페이지 태블릿 화면 (UI가 튀지 않고, 글자 크기가 매우 안정적)">
    <figcaption>내친 김에 태블릿 화면도 메인과 상세조회 둘 다 조금씩 손봤다.</figcaption>
</figure>

### 3. 카테고리 모음 화면 ([slug]/page.js)
다음은 카테고리 모음 화면이었다.

<figure>
    <img src="/images/cate_ex.png" alt="수정하기 이전의 카테고리 페이지 모바일 화면 (글자 크기가 매우 커서 UI가 찌그러짐)">
    <figcaption>수정 전에는 글자 크기때문에 UI가 망가져있었다.</figcaption>
</figure>

여타 `page.js`와 마찬가지로 마진과 패딩을 손보고 기본적으로 글자 크기를 잘 수정해줌으로써 비율을 맞춰보았다.

<figure>
    <img src="/images/cate_c.png" alt="수정된 카테고리 모음 페이지 모바일 화면 (글자 크기가 매우 안정적)">
    <figcaption>수정하고 나니 모바일 화면이 아주 그럴듯해졌다.</figcaption>
</figure>


<figure>
    <img src="/images/tab_c.png" alt="수정된 카테고리 모음 페이지 태블릿 화면 (글자 크기가 매우 안정적)">
    <figcaption>테블릿 화면도 아주 맘에 든다.</figcaption>
</figure>

### 4. 메인 layout.js의 header와 footer
이 역시 마진과 패딩, 글자 크기의 문제가 기본이었고, 검색창 컴포넌트인 `SearchModal.js`를 `md` 이상부터만 보여주도록 `hidden md:block` 미디어 쿼리를
작성했다.

그리고 태블릿 화면에선 `SearchModal.js`에 대한 위치 조정을 추가로 작업했다.

<figure>
    <img src="/images/header_c.png" alt="수정된 모바일 헤더 모습 (글끌 크기가 적절하며 검색창은 안 보임)">
    <figcaption>적절한 수정을 거치고 난 뒤 모바일 헤더 모습. 검색창은 더이상 보이지 않는다.</figcaption>
</figure>

<figure>
    <img src="/images/header_t.png" alt="수정된 태블릿 헤더 모습 (글끌 크기가 적절하며 검색창 보임)">
    <figcaption>적절한 수정을 거치고 난 뒤 태블릿 헤더 모습. 검색창 위치가 적절하다.</figcaption>
</figure>

### 마무리

처음엔 "그냥 사이드바만 숨기면 되지 않을까?"라고 단순하게 생각했지만, 막상 작업을 시작해보니 생각보다 챙겨야 할 디테일이 많았다. 글꼴 크기, 여백, 컴포넌트의 배치 방식까지 하나하나 손을 대면서 <b>사용자 경험(UX)</b>에 대해 다시 한번 깊게 고민해볼 수 있었다.

특히 Tailwind CSS의 미디어 쿼리는 정말 강력했다. `md:`, `lg:` 같은 짧은 접두사만으로 복잡한 반응형 로직을 직관적으로 처리할 수 있었고, 덕분에 기존 코드를 완전히 갈아엎지 않고도 우아하게 모바일 환경을 지원할 수 있었다.

이제 내 블로그는 지하철에서 스마트폰으로 보든, 카페에서 아이패드로 보든, 집에서 데스크탑으로 보든 언제나 최적의 가독성을 제공한다.
비록 모바일 퍼스트로 시작하지는 않았지만, 이번 리팩토링을 통해 어떤 환경에서도 유연하게 대응하는 웹 페이지로 거듭날 수 있었다.

반응형 웹 디자인, 처음엔 막막할 수 있지만 <b>"가장 작은 화면부터 하나씩 쌓아 올린다"</b>는 마음으로 접근하면 생각보다 즐거운 여정이 될 것이다.
이제 내 블로그도 손안의 작은 세상으로 한 걸음 더 다가갔다!