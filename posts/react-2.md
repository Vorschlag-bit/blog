...
5.  <b>Paint (Repaint)</b>: 픽셀을 화면에 그림
6.  **Composite**: 레이어 합성

### 왜 Reflow가 문제인가?
가장 큰 성능 저하 요인은 <b>Reflow(Layout)</b> 단계와 <b>싱글 스레드(Single Thread)</b> 환경이다.

브라우저의 메인 스레드(Main Thread)는 자바스크립트 실행과 화면 렌더링을 모두 담당한다. 즉, 자바스크립트가 복잡한 연산을 하거나 DOM을 조작하는 동안에는 화면을 그릴 수 없고, 반대로 화면을 그리는 동안에는 스크립트 실행이 멈춘다.

만약 자바스크립트로 DOM을 조작하다가 실수로 <b>강제 동기 레이아웃(Layout Thrashing)</b>을 유발하면 치명적인 성능 저하가 발생한다.

> **강제 동기 레이아웃이란?**  
> 브라우저는 최적화를 위해 스타일 변경을 큐(Queue)에 모았다가 한 번에 처리하려 한다. 하지만 자바스크립트에서 변경 직후 `offsetHeight`나 `scrollTop` 같은 **계산된 스타일**을 읽으려 하면, 브라우저는 정확한 값을 주기 위해 큐를 무시하고 **즉시 Reflow를 수행**해야 한다.
...
1.  **Render Phase**: 새로운 가상 돔을 만들고 이전 것과 비교(Diffing)하여 변경 사항을 수집한다. (순수 계산 영역)
2.  **Commit Phase**: 계산된 최종 변경 사항만 실제 DOM에 **한 번에** 적용한다.
