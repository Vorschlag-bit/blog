...
`load_mnist()` 함수의 인수인 `normalize`를 `True`로 설정하면 $0 \sim 255$ 범위의 각 픽셀의 값을 $0.0 \sim 1.0$ 범위의 값으로 변환한다. 이처럼 데이터를 특정 범위로 변환하는 처리를 <b>정규화(normalization)</b>라고 하고, 신경망의 입력 데이터에 특정 변환을 가하는 작업을 <b>전처리(pre-processing)</b>라 한다. 여기선 입력 이미지 데이터에 대한 전치리 작업으로 정규화를 수행한 셈.

현에서도 딥러닝에는 전처리를 활발히 사용한다. 여기선 단순히 각 픽셀의 값을 255로 나누는 단순한
정규화를 수행했으나, 현업에서는 데이터 전체의 분포를 고려해 전처리하는 경우가 많다.

예를 들어 데이터 전체 평균과 표준편차를 이용해 데이터들이 $0$을 중심으로 분포하도록 이동하거나
데이터의 확산 범위를 제한하는 정규화를 수행한다.

그 외에도 전체 데이터를 균일하게 분포시키는 데이터 <b>백색화(whitening)</b> 등도 있다.

### 변수가 갖는 물리적 의미
앞서 배운 배열의 형상에 주의해서 784개의 픽셀로 구성된 1개의 이미지가 우리가 원했던 출력층(0-9)로 나오는 과정을 행렬의 곱셈으로 요약하면 아래와 같다.

$$
X(1×784)\ ×\ W1(784×50)\ ×\ W2(50×100)\ ×\ W3(100×10)\ =\ y(10)
$$

행렬의 곱셈 원칙에 따라서 최종적으로 $1 × 10$ 크기의 행렬이 될 것이다.  
하지만 이렇게 숫자적으로만 이 형상을 바라보는 건 뭔가 직관적으로 와닿질 않는다.

각 변수($X, W1, W2, W3, Y$)가 물리적으로 어떤 의미를 갖는지를 구체적으로 알아볼 필요가 있다.

- $x(1×784)$: 입력 데이터 (사진 그 자체)
    - 가로 28, 세로 28 픽셀의 흑백 숫자 이미지이다.
    - 컴퓨터는 그림을 2차원 네모가 아니라 1차원 배열($1×784$)로 받아들인다.
    - 이 배열은 곧 점과 같고 각각 희고, 검다 등의 정보를 갖고 있다.
- $w_1,w_2,w_3$: 가중치 (필터)
    - 단순한 행렬 곱셈 같지만, <b>실제로는 정보를 요약하고 특징을 찾아내는 거름망</b>과 같다.
    - $w_1(784×50)$: 784개의 정보를 섞어 <b>50가지 기초 특징</b>을 찾아낸다.
    - $w_2(50×100)$: 50개의 기초 정보를 조합해 <b>100가지 복합 특징</b>을 찾아낸다.
    - $w_3(100×1)$: 100개의 복합 정보를 분석해 우리가 설정한 출력층과 연결한다.

위의 과정을 이해가 좀 더 쉽게 되도록 플로우 차트로 정리해봤다.

```mermaid
graph TD
    subgraph INPUT [입력 단계: x]
        IMG(이미지 '7') --> FLAT["1줄로 펴진 데이터<br/>(1 x 784)"]
        style IMG fill:#f9f,stroke:#333
        style FLAT fill:#fff,stroke:#333
    end

    subgraph L1 [첫 번째 변환: w1]
        W1("가중치 w1<br/>(784 x 50)")
        FLAT -- "행렬 곱셈" --> HIDDEN1["중간 특징 1<br/>(1 x 50)"]
        style HIDDEN1 fill:#ff9,stroke:#333
        
        note1[/"🔍해석: 픽셀들을 분석해<br/>'가로 선', '대각선' 같은<br/>50개의 기초 부품을 찾음"/]
        HIDDEN1 -.- note1
    end

    subgraph L2 [두 번째 변환: w2]
        W2("가중치 w2<br/>(50 x 100)")
        HIDDEN1 -- "행렬 곱셈" --> HIDDEN2["중간 특징 2<br/>(1 x 100)"]
        style HIDDEN2 fill:#ff9,stroke:#333
        
        note2[/"🧩해석: 기초 부품을 조립해<br/>'꺾인 모서리', '교차점' 같은<br/>100개의 복합 패턴을 만듦"/]
        HIDDEN2 -.- note2
    end

    subgraph OUT [출력 단계: y]
        W3("가중치 w3<br/>(100 x 10)")
        HIDDEN2 -- "행렬 곱셈" --> OUTPUT["최종 점수표<br/>(1 x 10)"]
        style OUTPUT fill:#9f9,stroke:#333
        
        RESULT[/"결과:<br/>[0, 0, 0, 0, 0, 0, 0, 0.9, 0, 0.1]<br/>(7번 칸의 점수가 제일 높음!)"/]
        OUTPUT -.- RESULT
    end
...
```
 ...
 ### 배치 처리
 만약 입력 데이터의 형상을 $100×784$로 준다면, 출력 데이터의 형상은 $100×10$이 될 것이다.
 이는 100장 분량의 입력 데이터 결과가 한 번에 출력된다는 걸 의미한다.

 예를 들어 `y[0][0]`에는 0번째 이미지와 0과의 추론 결과가 담길 것이다.

 이처럼 하나로 묶은 입력 데이터를 <b>배치(batch)</b>라고 한다. 배치 처리는 컴퓨터로 계산 시
 큰 이점을 준다. 이미지 1장당 처리 시간을 대폭 줄여주기 때문이다. 이게 가능한 이유는 2가지로,
 하나는 수치 계산 라이브러리 대부분이 큰 배열을 효율적으로 처리할 수 있도록 고도로 최적화되어
 있기 때문이다. 
 
 그리고 커다란 신경망에선 데이터 전송이 병목으로 작용하는 경우가 잦은데, 배치 처리를 함으로써 부하는 줄이는 것이 두 번째 이유이다. (느린 I/O를 통해 데이터를 읽는 횟수 자체가 줄어 순수 계산 비율이 높아짐)

 즉, 배치 처리를 수행함으로써 큰 배열로 이뤄진 계산에 있어서 컴퓨터는 큰 배열을 한 번에 계산하는 게 작은 배열을 여러 번 계산할 때보다 빠르다.

 ## 정리
 지금까지 신경망의 순전파에 대해서 알아보았다. 신경망은 각 층의 뉴런들이 다음 층의 뉴런으로
 신호를 전달한다는 점에서 앞 장의 퍼셉트론과 일치하나 다음 뉴런으로 갈 때
 신호를 변화시키는 <b>활성화 함수</b>에서 큰 차이가 있었다.

 신경망에서는 매끄럽게 변화하는 시그모이드 함수를, 퍼셉트론에서는 갑자기 변화하는 계단 함수를
 활성화 함수로 사용했다.

 매끄럽다는 표현이 가져다주는 의미에 대해서는 다음 글에 이어서 학습할 예정.
 ...
 - 신경망에서는 활성화 함수로 시그모이드 함수와 ReLU 함수 같이 <b>매끄럽게 변화하는 함수</b>를 이용한다.
 - 넘파이의 다차원 배열을 사용하면 신경망을 효율적으로 구현 가능하다.
 - 머신러닝 문제는 크게 회귀와 분류로 나눌 수 있다.
 - 출력층의 활성화 함수로는 <b>회귀에서는 주로 항등 함수</b>를, <b>분류에서는 주로 소프트맥스 함수</b>를 사용한다.
 - 분류에서는 출력층의 뉴런 수를 분류하려는 클래스 수와 같게 설정한다.
 - 입력 데이터를 묶은 걸 배치라 하며, 추론 처리를 이 배치 단위로 설정하면 컴퓨터의 계산 속도는 비약적으로 상승한다.