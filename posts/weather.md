...
<span style="color: red">※ 항목구분 : 필수(1), 옵션(0), 1 건 이상 복수건(1..n), 0 건 또는 복수건(0..n), 코드표별첨</span>
...
여기서 내게 필요한 것은 **기온**, **강수형태**, **습도**, **1시간 강수량** 정도일 것이다.  
...
따라서 동단위의 정확도와 성능(캐싱)을 모두 잡기 위해선 **클라이언트의 GPS**와 **서버 캐싱**을 잘 설계할 필요가 있었다.
...
<strong>위도(Latitude, 세로)</strong>는 어디서나 간격이 일정하다.  
따라서 계산하면 $$ 40,000\text{km} \div 360 \approx 111\text{km} (111,195\text{m}) $$.
...
| 소수점 자리 | 값의 크기 | 위도 거리 (세로) | 경도 거리 (가로) |
| :--- | :--- | :--- | :--- |
| 1자리 (0.1) | 1/10 | 11.1 km | 8.9 km |
| 2자리 (0.01) | 1/100 | 1.1 km | 890 m |
| **3자리 (0.001)** | **1/1000** | **111 m** | **89 m** |
| 4자리 (0.0001) | 1/10000 | 11 m | 8.9 m |
| 5자리 (0.00001)| 1/100000 | 1.1 m | 0.89 m |
| ... | ... | ... | ... |
| 14자리 | - | 나노미터 단위 | - |
